#PART 1 - Cleaning already existing 
library(ggplot2)
library(maps)
library(stringr)
library(tidyr)
...
#NAs for all - same states as for corn production
na_corn_prices_states <- merge_final %>%
  group_by(state) %>%
  filter(all(is.na(corn.prices))) %>%
  pull(state) %>%
  unique()
#imputing 0s if all NAs
merge_final$corn.prices[merge_final$state %in% na_corn_production_states & is.na(merge_final$corn.prices)] = 0
...
write.csv(merge_final, "Data/Merging/merge_final_cleaning_Indira.csv")